%% 6 de agosto 2025
%Directorio: C:\Users\Marco\Documents\MATLAB\EMG\Capgmyo\db_a\sEMG_capgmyo_3_test_estadistic_v2

% incluye PRUEBA ESTADÍSTICA...Friedman + post-hoc de Nemenyi. 
% codigo original para LA BDD "CapgMyo"
...
repeticiones=10;
electrodos=128;
f_original=1000; f_nueva=100;
factor = f_original / f_nueva;  % Factor de resampleo
%tic
data_capgmyo=f_resample_capgmyo(sujetos,gestos,repeticiones,electrodos,factor); %data CAPGMYO resampleada a 100 Hz.
%toc
%delete(gcp)
[n_sujetos, n_gestos, n_repeticiones, n_muestras, n_electrodos] = size(data_capgmyo); %obtengo dimensiones de matriz
features_list={'RMS','MAV','VAR','WL','HIST','CC','WAV'};
% Preasignar la celda data_users con el número de sujetos
data_users = cell(sujetos, 1);
...
for i = 1:sujetos
    fprintf('\n\nCapgMyo - Sujeto: %d', i);
    emg = reshape(data_capgmyo(i, :, :, :, :),[n_gestos, n_repeticiones, n_muestras, n_electrodos]);
...
% data_capgmyo es ahora 135×6
% Nombres de columnas (para referencia):
nombres = {'KNN','SVM','LDA','RF','MLP','LSTM'};

data_capgmyo_st=zeros(sujetos*5,6); % (sujetos x folds, clasificadores)
count=0;
for i=1:sujetos
    data_capgmyo_st(count+1:count+5,:)=data_users{i,1}.acc_users;
    count=count+5;
end
[p_friedman, tbl, stats] = friedman(data_capgmyo_st, 1, 'off');
fprintf('\n\n\n=== CAPGMYO DBa ===\n');
...
if p_friedman < 0.05
    ...
    p_nemenyi = f_nemenyi_test(data_capgmyo_st);
    f_mostrar_nemenyi(nombres,p_nemenyi,data_capgmyo_st);
else
    fprintf('→ No hay diferencia significativa entre clasificadores.\n\n');
end

%% funcion para dibujar la matriz de comfusion promedio
...
for i=1:sujetos
    ...
    p_knn_VAR(i)=data_users{i,1}.p_knn_VAR; r_knn_VAR(i)=data_users{i,1}.r_knn_VAR; s_knn_VAR(i)=data_users{i,1}.s_knn_VAR; f1_knn_VAR(i)=data_users{i,1}.f1_knn_VAR;
    p_svm_VAR(i)=data_users{i,1}.p_svm_VAR; r_svm_VAR(i)=data_users{i,1}.r_svm_VAR; s_svm_VAR(i)=data_users{i,1}.s_svm_VAR; f1_svm_VAR(i)=data_users{i,1}.f1_svm_VAR;
    p_lda_VAR(i)=data_users{i,1}.p_lda_VAR; r_lda_VAR(i)=data_users{i,1}.r_lda_VAR; s_lda_VAR(i)=data_users{i,1}.s_lda_VAR; f1_lda_VAR(i)=data_users{i,1}.f1_lda_VAR;
    p_rf_VAR(i)=data_users{i,1}.p_rf_VAR; r_rf_VAR(i)=data_users{i,1}.r_rf_VAR; s_rf_VAR(i)=data_users{i,1}.s_rf_VAR; f1_rf_VAR(i)=data_users{i,1}.f1_rf_VAR;
    p_mlp_VAR(i)=data_users{i,1}.p_mlp_VAR; r_mlp_VAR(i)=data_users{i,1}.r_mlp_VAR; s_mlp_VAR(i)=data_users{i,1}.s_mlp_VAR; f1_mlp_VAR(i)=data_users{i,1}.f1_mlp_VAR;
    p_lstm_VAR(i)=data_users{i,1}.p_lstm_VAR; r_lstm_VAR(i)=data_users{i,1}.r_lstm_VAR; s_lstm_VAR(i)=data_users{i,1}.s_lstm_VAR; f1_lstm_VAR(i)=data_users{i,1}.f1_lstm_VAR;
end
...
% fprintf('\n\nVAR-KNN (media ± std)==> \tPrecision: %0.2f±%0.2f \tRecall: %0.2f±%0.2f \tSensitivity: %0.2f±%0.2f \tF1-score: %0.2f±%0.2f',mean(p_knn_VAR),std(p_knn_VAR),mean(r_knn_VAR),std(r_knn_VAR),mean(s_knn_VAR),std(s_knn_VAR),mean(f1_knn_VAR),std(f1_knn_VAR));
% fprintf('\n\nVAR-SVM (media ± std)==> \tPrecision: %0.2f±%0.2f \tRecall: %0.2f±%0.2f \tSensitivity: %0.2f±%0.2f \tF1-score: %0.2f±%0.2f',mean(p_svm_VAR),std(p_svm_VAR),mean(r_svm_VAR),std(r_svm_VAR),mean(s_svm_VAR),std(s_svm_VAR),mean(f1_svm_VAR),std(f1_svm_VAR));
% fprintf('\nVAR-LDA (media ± std)==> \tPrecision: %0.2f±%0.2f \tRecall: %0.2f±%0.2f \tSensitivity: %0.2f±%0.2f \tF1-score: %0.2f±%0.2f',mean(p_lda_VAR),std(p_lda_VAR),mean(r_lda_VAR),std(r_lda_VAR),mean(s_lda_VAR),std(s_lda_VAR),mean(f1_lda_VAR),std(f1_lda_VAR));
% fprintf('\nVAR-RF (media ± std)==> \tPrecision: %0.2f±%0.2f \tRecall: %0.2f±%0.2f \tSensitivity: %0.2f±%0.2f \tF1-score: %0.2f±%0.2f',mean(p_rf_VAR),std(p_rf_VAR),mean(r_rf_VAR),std(r_rf_VAR),mean(s_rf_VAR),std(s_rf_VAR),mean(f1_rf_VAR),std(f1_rf_VAR));
% fprintf('\nVAR-MLP (media ± std)==> \tPrecision: %0.2f±%0.2f \tRecall: %0.2f±%0.2f \tSensitivity: %0.2f±%0.2f \tF1-score: %0.2f±%0.2f',mean(p_mlp_VAR),std(p_mlp_VAR),mean(r_mlp_VAR),std(r_mlp_VAR),mean(s_mlp_VAR),std(s_mlp_VAR),mean(f1_mlp_VAR),std(f1_mlp_VAR));
% fprintf('\nVAR-LSTM (media ± std)==> \tPrecision: %0.2f±%0.2f \tRecall: %0.2f±%0.2f \tSensitivity: %0.2f±%0.2f \tF1-score: %0.2f±%0.2f',mean(p_lstm_VAR),std(p_lstm_VAR),mean(r_lstm_VAR),std(r_lstm_VAR),mean(s_lstm_VAR),std(s_lstm_VAR),mean(f1_lstm_VAR),std(f1_lstm_VAR));
...
for i=1:sujetos
    ...
    tt_lstm_RMS(i)=data_users{i,1}.tt_lstm_RMS;
    ...
    tt_lstm_MAV(i)=data_users{i,1}.tt_lstm_MAV;
    ...
    pt_lstm_RMS(i)=data_users{i,1}.pt_lstm_RMS;
    ...
    pt_lstm_MAV(i)=data_users{i,1}.pt_lstm_MAV;
end
...
f_guarda_datos(a_knn_RMS, a_svm_RMS, a_lda_RMS, a_rf_RMS, a_mlp_RMS, a_lstm_RMS, ...
    ...
    mtx_mean_cm_mav_lda,mtx_mean_cm_mav_lstm,data_capgmyo_st)

fprintf('\n\nProcesamiento Finalizado\n'); 
delete(gcp)

%registro de accuracy y generacion de archivo .CSV para posterior analisis.
tabla_accuracy= f_lectura_accuracies(a_knn_RMS,a_knn_MAV,a_knn_VAR,a_svm_RMS,a_svm_MAV,a_svm_VAR,...
        a_lda_RMS,a_lda_MAV,a_lda_VAR,a_rf_RMS,a_rf_MAV,a_rf_VAR,...
        a_mlp_RMS,a_mlp_MAV,a_mlp_VAR,a_lstm_RMS,a_lstm_MAV,a_lstm_VAR);

% save('tabla_accuracy.mat','tabla_accuracy');
% writetable(tabla_accuracy,'tabla_accuracy.csv');
% fprintf('\nResultados de accuracy guardados en tabla_accuracy.csv\n');
...
function [mtx_WL]=extraer_signal_WL(emg,gestos,repeticiones,electrodos)
    mtx_WL=[]; fs=100; 
    current_row = 1; max_diff=9; salto= electrodos; %10;  %cantidad de muestras por electrodo 
    for i=1:gestos % recorro etiquetas (8 etiquetas)
        %label_name=sprintf('label_%d',i); %selecciono etiqueta
        wl_ini=zeros(electrodos,max_diff); current_row1=1; salto1=electrodos; features2=zeros(electrodos,max_diff);
        for j=1:repeticiones %recorro repeticiones (10 repeticiones)
            for k=1:electrodos %recorro electrodos (128 electrodos)
        ...
        % [~,indx_elec]=f_best10_RMS_MAV(features2,10);% selecciono las 10 señales con mayor RMS y MAV.
        % rep=1:repeticiones;rep=randi(length(rep)); % selecciono al azar una repeticion.
        % features2=squeeze(emg(i,rep,:,indx_elec));
        % label(1:size(features2,2))=i; %etiqueto
        % features1=[features2' label'];
...
function [mtx_HIST]=HIST_features(emg,gestos,repeticiones,electrodos)
    ...
    for i=1:gestos % recorro etiquetas
        ...
        % [~,indx_elec]=f_best10_var(features2,10);%selecciono las 10 señales con mayor VARIANZA.
        % rep=1:repeticiones;rep=randi(length(rep)); % selecciono al azar una repeticion.
        % features2=squeeze(emg(i,rep,:,indx_elec));
        % label(1:size(features2,2))=i; %ETIQUETO
        % features3=[features2' label'];
        features4(current_row1:current_row1+salto1-1,:)=features3;%guardo todas las matrices que genero con wl
        current_row1 = current_row1 + salto1;
    end
    mtx_HIST=features4;
end
...
function [mtx_CC]=CC_features(emg,gestos,repeticiones,electrodos) % coeficientes cepstrales
    ...
    for i=1:gestos % recorro etiquetas
        ...
        % [~,indx_elec]=f_best10_var(features1,10);%selecciono las 10 señales con mayor VARIANZA.
        % rep=1:repeticiones;rep=randi(length(rep)); % selecciono al azar una repeticion.
        % features1=squeeze(emg(i,rep,:,indx_elec));
        % label(1:size(features1,2))=i; % ETIQUETO
        % features2=[features1' label'];
...
function [mtx_coefApprox]=DWT_features(emg,waveletName,numLevels,gestos,repeticiones,electrodos)
    ...
    for i=1:gestos % recorro etiquetas
        features1=[];
        for j=1:repeticiones %recorro repeticiones (10 repeticiones)
            feat_coefApprox=[];%feat_coefApprox=zeros(1,max_diff*10);
            for k=1:electrodos %recorro electrodos (10 electrodos)
                segment(:,1) = emg(i,j,:,k); % Extraer segmento de señal
                ...
                feat_coefApprox=vertcat(feat_coefApprox,coefApprox);
            end
...
function[acc_knn,acc_svm,acc_lda,acc_rf,acc_mlp,acc_lstm,...
    ...
    cv =  cvpartition(Y, 'KFold', 5, 'Stratify', true); % 5 folds estratificados    
...
%for i=1:num_folds
        ...
        XTrainLSTM = f_convert_to_lstm_format(Xtrain);
        ...
        num_classes = 8;      % 8 gestos  %%===>>>>> CORRECCIÓN PARA BDD CAPGMYO <<<<<<===%%
