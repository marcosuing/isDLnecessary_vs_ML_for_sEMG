function [tabla_accuracy] = f_lectura_accuracies(a_knn_RMS,a_knn_MAV,a_knn_VAR,a_svm_RMS,a_svm_MAV,a_svm_VAR,...
    ...
    % a_mlp_MAV=a_mlp_MAV;
    % a_mlp_VAR=a_mlp_VAR;
    % a_lstm_RMS=a_lstm_RMS;
    % a_lstm_MAV=a_lstm_MAV;
    % a_lstm_VAR=a_lstm_VAR;
    ...
    n_muestras = 11; % Dimensión de cada vector (1x11)
    
    % Preasignación de espacio para la matriz (18 filas x 11 columnas)
        ...
        for j = 1:n_desc
            ...
            if exist(nombre_var, 'var')
                ...
                if exist(nombre_var_alt, 'var')
                    ...
                    warning('\nNo se encontró la variable "%s" ni "%s" en el Workspace.', nombre_var, nombre_var_alt);
                end
            end
    ...
    nombre_csv = 'resultados_accuracy_db3_17_sep_2026.csv';
    
    % OPCIÓN A (Recomendada): Guarda la tabla con nombres de columna
    writetable(tabla_accuracy, nombre_csv);
    
    % OPCIÓN B (Opcional): guardar la matriz numérica pura (sin encabezados)
    ...
    fprintf('\n¡Matriz exportada exitosamente como "%s"!\n', nombre_csv);
