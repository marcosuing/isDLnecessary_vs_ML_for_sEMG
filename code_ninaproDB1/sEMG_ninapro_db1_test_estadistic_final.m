...
%Directorio: C:\Users\Marco\Documents\MATLAB\EMG\Ninapro\sEMG_ninapro_db1_test_estadistic_v2

% incluye PRUEBA ESTADÍSTICA...Friedman + post-hoc de Nemenyi. 
% codigo para analizar la BDD Ninapro DB1 (con amputados)
% fs=100 Hz. duracción de movimiento= 5seg + 3 seg de descanso.
% Emg (12 columnas): 
...
%% Directorio Archivos
base_folder = 'C:\Users\Marco\Documents\MATLAB\EMG\Ninapro';
sujetos = 27;
gestos = 12;
repeticiones = 10;
electrodos=10;
...
for u = 1:sujetos
    user_folder = fullfile(base_folder, sprintf('s%d', u));
    %user_folder = fullfile(base_folder, sprintf('s%d_0', u),sprintf('DB3_s%d',u));
    file_path = fullfile(user_folder, sprintf('S%d_A1_E1.mat', u));
...
%tic
data_ninapro_db1=f_resample_ninaprodb1(all_data,rng_etiquetasUser,min_diff,sujetos,gestos,repeticiones,electrodos,factor);
%toc
[n_sujetos, n_gestos, n_repeticiones, n_muestras, n_electrodos] = size(data_ninapro_db1); %obtengo dimensiones de matriz
...
for i = 1:sujetos
    fprintf('\n\nDB1 - Sujeto: %d', i);
    emg = reshape(data_ninapro_db1(i, :, :, :, :),[n_gestos, n_repeticiones, n_muestras, n_electrodos]);
    ...
    mtx_mean_cm_mav_lda(:,:,i)=cm_promedio_mav_lda; %para matriz de confusión promedio

    [cm_promedio_mav_lstm]=f_clasific_MAV_lstm(balanced_features.MAV);
    mtx_mean_cm_mav_lstm(:,:,i)=cm_promedio_mav_lstm; %para matriz de confusión promedio
...
% data_db1 es ahora 135×6
% Nombres de columnas (para referencia):
nombres = {'KNN','SVM','LDA','RF','MLP','LSTM'};

data_db1=zeros(sujetos*5,6); % (sujetos x folds, clasificadores)
count=0;
for i=1:sujetos
    data_db1(count+1:count+5,:)=data_users{i,1}.acc_users;
    count=count+5;
end
[p_friedman, tbl, stats] = friedman(data_db1, 1, 'off');
fprintf('\n\n\n=== NINAPRO DB1 ===\n');
...
if p_friedman < 0.05
    ...
    p_nemenyi = f_nemenyi_test(data_db1);
    f_mostrar_nemenyi(nombres,p_nemenyi,data_db1);
else
...
%GRÁFICA QUE COMPARA ACCURACY DE ML Y DL, con lo metodos de caracterización
...
%FUNCIÓN PARA GUARDAR DATOS GENERADOS
    ...
    for i=1:gestos % recorro etiquetas 
        ...
        for j=1:repeticiones %recorro repeticiones 
            for k=1:electrodos %recorro electrodos 
                segment(:,1) = emg(i,j,:,k); % Extraer segmento de señal. Gesto-repeticion-muestra-electrodo
                aux=f_wl(segment,fs);
...
parfor i=1:num_folds
        ...
        num_classes = 12;      % 17 gestos  %%===>>>>> CORRECCIÓN PARA BDD NINAPRO DB3 <<<<<<===%%
